...
<main style="width: 760px; margin-left: 20px;">
    ...
    <div style="padding: 0 15%;">
        <h1 style="color: #000;font-size:16px;margin-bottom:20px !important;text-align:center;">Rolling Return </h1>
    </div>
    ...
    <table class="table text-center mfliketbl" style="margin-top:10px;">
        <tbody>
            <tr>
                <th rowspan="2" style="width: 190px;"><strong>Scheme</strong></th>
                @if($period1_id)
            ...
            <?php foreach($rolling_return as $key=>$value){ 
                ...
                <tr>
                    ...
                    @if($period1_id)
                        <td style="text-align:left;">{{number_format((float)($value['avg_1']), 2, '.', '')}}</td>
                        <td style="text-align:left;">{{number_format((float)($value['min_1']), 2, '.', '')}}</td>
                        <td style="text-align:left;">{{number_format((float)($value['max_1']), 2, '.', '')}}</td>
                    @endif
                    @if($period2_id)
                        <td style="text-align:left;">{{number_format((float)($value['avg_2']), 2, '.', '')}}</td>
                        <td style="text-align:left;">{{number_format((float)($value['min_2']), 2, '.', '')}}</td>
                        <td style="text-align:left;">{{number_format((float)($value['max_2']), 2, '.', '')}}</td>
                    @endif
                    @if($period3_id)
                        <td style="text-align:left;">{{number_format((float)($value['avg_3']), 2, '.', '')}}</td>
                        <td style="text-align:left;">{{number_format((float)($value['min_3']), 2, '.', '')}}</td>
                        <td style="text-align:left;">{{number_format((float)($value['max_3']), 2, '.', '')}}</td>
                    @endif
                </tr>
            <?php } ?>
        </tbody>
