...
@section('js_after')
    ...
    <script>
        ...
        var global_planwise_scheme_list = [];
        global_scheme_list.forEach(function(val){
            if(global_planwise_scheme_list[val.plan]){
                global_planwise_scheme_list[val.plan].push(val);
            }else{
                global_planwise_scheme_list[val.plan] = [];
                global_planwise_scheme_list[val.plan].push(val);
            }
            
        });

        console.log("global_planwise_scheme_list");
        console.log(global_planwise_scheme_list);
        ...
        function changePlan(){
            ...
            var s_plan_id = document.getElementById("s_plan_id").value;

            var form_list = document.getElementById("schemecode_view").querySelectorAll('select[message=text]');
            var iHtml = `<option value=""> </option>`;
            global_planwise_scheme_list[s_plan_id].forEach(function(val){
                iHtml = iHtml+`<option value="`+val.schemecode+`">`+val.s_name+`</option>`;
            });

            form_list.forEach(function(val){
                var select_id = $(val).attr('id');

                document.getElementById(select_id).innerHTML = iHtml;
            });
        ...
        function renderRow(all_data){
            ...
            global_result.forEach(function(val){
                ...
                if(all_data.period1_id){
                    valueee = "-";
                    if(val.avg_1){
                        valueee = Number((parseFloat(val.avg_1)).toFixed(2))
                    }
                    table_html = table_html+'<td >'+valueee+'</td>';
                    valueee = "-";
                    if(val.max_1){
                        valueee = Number((parseFloat(val.max_1)).toFixed(2))
                    }
                    table_html = table_html+'<td >'+valueee+'</td>';
                    valueee = "-";
                    if(val.min_1){
                        valueee = Number((parseFloat(val.min_1)).toFixed(2))
                    }
                    table_html = table_html+'<td >'+valueee+'</td>';
                }
                if(all_data.period2_id){
                    valueee = "-";
                    if(val.avg_2){
                        valueee = Number((parseFloat(val.avg_2)).toFixed(2))
                    }
                    table_html = table_html+'<td >'+valueee+'</td>';
                    valueee = "-";
                    if(val.max_2){
                        valueee = Number((parseFloat(val.max_2)).toFixed(2))
                    }
                    table_html = table_html+'<td >'+valueee+'</td>';
                    valueee = "-";
                    if(val.min_2){
                        valueee = Number((parseFloat(val.min_2)).toFixed(2))
                    }
                    table_html = table_html+'<td >'+valueee+'</td>';
                }
                if(all_data.period3_id){
                    valueee = "-";
                    if(val.avg_3){
                        valueee = Number((parseFloat(val.avg_3)).toFixed(2))
                    }
                    table_html = table_html+'<td >'+valueee+'</td>';
                    valueee = "-";
                    if(val.max_3){
                        valueee = Number((parseFloat(val.max_3)).toFixed(2))
                    }
                    table_html = table_html+'<td >'+valueee+'</td>';
                    valueee = "-";
                    if(val.min_3){
                        valueee = Number((parseFloat(val.min_3)).toFixed(2))
                    }
                    table_html = table_html+'<td >'+valueee+'</td>';
                }
                table_html = table_html+"</tr>";
        ...
        function renderSchemeRow(all_data,){
            var table_html = ``;
            table_html = "<thead>";
            table_html = table_html+"<tr>";
            table_html = table_html+'<th rowspan="2" style="text-align: left;">Scheme</th>';
        ...
        function addData(){
            ...
            var form_list = document.getElementById("schemecode_view").querySelectorAll('select[message=text]');
            form_list.forEach(function(val){
                data.schemecode_id.push($(val).val());
            });
        ...
        function addSchemeData(){
            var form_list = document.getElementById("schemecode_view").querySelectorAll('select[message=text]');
            
            if(form_list.length < 20){
                var s_plan_id = document.getElementById("s_plan_id").value;
                ...
                index_scheme = index_scheme + 1;
                var iHtml = `<div class="input-group row" style="margin-bottom: 10px;" id="schemecode_view_`+index_scheme+`">
                                <label for="schemecode_id" class="col-sm-3 col-form-label">Select Scheme</label>
                                <div class="col-sm-9">
                                    <select class="form-control ui-autocomplete-input schemecode_id" name="s_schemecode_id[`+index_scheme+`]" id="s_schemecode_id_`+index_scheme+`" onchange="changeScheme('`+index_scheme+`');"  message="text">
                                        `+iHtml+`
                                    </select>
                                    <em id="s_schemecode_id_error" class="error"></em>
                                    <div style="cursor: pointer;color: red;text-align: right;" onclick="removeSchemeData('`+index_scheme+`')">Remove</div>
                                </div> 
                            </div>`;

                $("#schemecode_view").append(iHtml);
        ...
        function removeSchemeData(index){
            document.getElementById("schemecode_view_"+index).remove();
        }
            ...
            <div class="row Search_Here" style="margin-top: 20px;">
                <div class="col-12 col-sm-10 col-md-8 col-lg-8 col-xl-8" >
                    <form method="get" action="{{route('frontend.rolling_return_action')}}" id="comp_sch" >
                        ...
                        <input type="hidden" id="shorting_id" name="shorting_id" value="">
                        ...
                        <div id="categorywise_type" style="">
                            <div>
                                ...
                                <div class="input-group row" style="margin-bottom: 10px;">
                                    ...
                                    <div class="col-sm-9">
                                        <select class="form-control ui-autocomplete-input" name="plan_id" id="plan_id">
                                            <option value="">Select</option>
                                ...
                                <div class="input-group-append text-center" style="display: flex; justify-content: center; padding-top: 16px;">
                                    ...
                                    @if (Auth::check())
                                        @if($permission['is_download'])
                                            <button type="submit" class="btn btn-primary" id="download_button" style="margin-left: 20px;" onclick="downloadButton();">Download</button>
                                        @else
                                            <button type="button" class="btn btn-primary" id="download_button" style="margin-left: 20px;" onclick="openDownloadPermissionModal();">
                                                Download
                                            </button>
                                        @endif
                                    @else
                                        <button type="button" class="btn btn-primary" id="download_button" style="margin-left: 20px;" onclick="openDownloadLoginModal();">
                                            Download
                                        </button>
                                    @endif
                                </div>  
                        ...
                        <div id="schemewise_type" style="display: none;">
                            <div>
                                ...
                                <div id="schemecode_view">
                                    <div class="input-group row" style="margin-bottom: 10px;" id="schemecode_view_0">
                                        <label for="schemecode_id" class="col-sm-3 col-form-label">Select Scheme</label>
                                        <div class="col-sm-9">
                                            <select class="form-control ui-autocomplete-input schemecode_id" name="s_schemecode_id[0]" id="s_schemecode_id_0" message="text" onchange="changeScheme(0);">
                                ...
                                <div class="input-group-append text-center" style="display: flex; justify-content: center; padding-top: 16px;">
                                    <button type="button" class="btn btn-primary add_button" id="add_btn_search" onclick="addSchemeData();">Add</button>
                                </div>

                                <div class="input-group-append text-center" style="display: flex; justify-content: center; padding-top: 16px;">
                                    <button type="button" class="btn btn-primary add_button" id="add_btn_search" onclick="addData();">Submit</button>
                                    @if (Auth::check())
                                        @if($permission['is_download'])
                                            <button type="submit" class="btn btn-primary" id="download_button" style="margin-left: 20px;" onclick="downloadButton();">Download</button>
                                        @else
