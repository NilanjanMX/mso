                        ...
                        @if(isset($suggested_scheme_list) && count($suggested_scheme_list)>0)
                            @foreach($suggested_scheme_list as $key=>$value)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <select class='number-wrapper' name='scheme_type[{{$key}}]' style='border: 1px solid #F1F3F4;height: 21px;color: #000;font-size: 13px;width: 88px;padding: 0px 5px;'>
                                            <option value='' @if(isset($scheme_type[$key])) @if($scheme_type[$key]=='') selected  @endif @endif></option>
                                            <option @if(isset($scheme_type[$key])) @if($scheme_type[$key]=='SIP') selected  @endif @endif>SIP</option>
                                            <option @if(isset($scheme_type[$key])) @if($scheme_type[$key]=='Lumpsum') selected  @endif @endif>Lumpsum</option>
                                        </select>
                                    <td>
                                    <span class='number-wrapper' style='padding: 0px 5px;'>
                                        <input type="number" name="scheme_amount[{{$key}}]" value="{{isset($scheme_amount[$key])?$scheme_amount[$key]:""}}"  style='width: 72px;'> ₹</span>
                                    </td>
                                    <td style="text-align: center;">
                                        <a href="#" class="text-danger remove-suggested-tr" dataid="{{$key}}" title="Remove"><img src="{{asset('')}}/f/images/delblack.png"></a>
                                        <input class="schemecode-input" type="hidden" name="schemecode[]" value="{{$suggested_scheme['ASSET_TYPE']}}_{{$key}}_{{$suggested_scheme['OPTION']}}" />
                                    </td>
