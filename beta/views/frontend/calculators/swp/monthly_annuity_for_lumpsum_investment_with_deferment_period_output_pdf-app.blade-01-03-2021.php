...
@if(!isset($accumulation_phase_interest_rate_2) || $accumulation_phase_interest_rate_2 == '')
    ...
    @if(!isset($accumulation_phase_interest_rate_2))
        ...
        <div style="">
            ...
            <table style="width: 532px; margin: 0 auto;">
                <tbody>
                @if(isset($accumulation_phase_interest_rate_2) && $accumulation_phase_interest_rate_2>0)
                    <tr>
                        <th style="width: 50%;">
    ...
    @php
        $note_data1 = \App\Models\Calculator_note::where('category','summery')->where('calculator','Monthly_Annuity_For_Lumpsum_Investment_With_Deferment_Period')->first();
        if(!empty($note_data1)){
        @endphp
    ...
    @if(isset($report) && $report=='detailed')
        ...
        @php
        $note_data2 = \App\Models\Calculator_note::where('category','cashflow')->where('calculator','Monthly_Annuity_For_Lumpsum_Investment_With_Deferment_Period')->first();
        if(!empty($note_data2)){
        @endphp
            ...
            @if(isset($distribution_phase_interest_rate_2) && $distribution_phase_interest_rate_2>0)
                ...
                @for($i=1;$i<=$annuity_period;$i++)
                    ...
                    <tr>
                        ...
                        <td><span style="font-family: DejaVu Sans; sans-serif;">&#8377;</span> {{custome_money_format($balance_eom_1)}}</td>
                        <td><span style="font-family: DejaVu Sans; sans-serif;">&#8377;</span> {{custome_money_format($monthly_annuity_amount2)}}</td>
                        <td><span style="font-family: DejaVu Sans; sans-serif;">&#8377;</span> {{custome_money_format($balance_eom_2)}}</td>
                    </tr>
