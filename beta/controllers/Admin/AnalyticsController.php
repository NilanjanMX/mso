...
class AnalyticsController extends Controller
{
    ...
    public function trail_calculator(Request $request){
        ...
        if($data['page_type'] == "DOWNLOAD"){
            ...
            $filename = "trail_calculator.csv";
            $handle = fopen("./storage/app/".$filename, 'w');
            fputcsv($handle, array('SN', 'Name', 'Total View'));
    ...
    public function other_download(Request $request){
        ...
        if($data['page_type'] == "DOWNLOAD"){
            ...
            $filename = "other_download.csv";
            $handle = fopen("./storage/app/".$filename, 'w');
            fputcsv($handle, array('SN', 'Name', 'Total Download'));
    ...
    public function calculator_scheme_wise(Request $request){
        ...
        if($data['date_type']){
            ...
            $data['reports'] = $data['reports']->whereBetween('histories.created_at', [$from , $to]);
        }else{
            if($data['ustart_date'] && $data['uend_date']){
                $date_from = explode('/', $data['ustart_date']);
                $date_to = explode('/', $data['uend_date']);
    ...
    public function calculator_suggested_schemes(Request $request){
        ...
        if($data['page_type'] == "DOWNLOAD"){
            ...
            $filename = "calculator_analytics.csv";
            $handle = fopen("./storage/app/".$filename, 'w');
            fputcsv($handle, array('SN', 'Scheme Name', 'View Count', 'DownLoad Count', 'Save Count', 'Total Count'));

            foreach($data['reports'] as $key=>$row) {

                fputcsv($handle, array($key+1, $row->page_name, $row->total_view_count, $row->total_download_count, $row->total_save_count,$row->total_list_count+$row->total_download_count+$row->total_save_count+$row->total_view_count));
            }
            // dd("row");
            fclose($handle);
    ...
    public function calculator_user_wise(Request $request){
        ...
        $data['reports'] = $data['reports']->where('page_type','=','Calculator')->whereNotNull('users.email')->groupBy(['user_id'])->get();
        ...
        if($data['page_type'] == "DOWNLOAD"){
            ...
            $handle = fopen("./storage/app/".$filename, 'w');
            fputcsv($handle, array('SN', 'User Name', 'User Email', 'User City','Package Name', 'List Count', 'View Count', 'DownLoad Count', 'Save Count', 'Total Count'));
            

            foreach($data['reports'] as $key=>$row) {

                fputcsv($handle, array($key+1, $row->user_name, $row->user_email, $row->user_city, $row->package_name, $row->total_list_count, $row->total_view_count, $row->total_download_count, $row->total_save_count,$row->total_list_count+$row->total_download_count+$row->total_save_count+$row->total_view_count));
    ...
    public function calculators(Request $request){
        ...
        $data['reports'] = DB::table("histories")->select(['histories.*','calculators.name as page_name','users.name as username','users.email as user_email'])->leftJoin('calculators', 'histories.page_id', '=', 'calculators.id')->leftJoin('users', 'histories.user_id', '=', 'users.id');
        ...
        if($data['page_type'] == "DOWNLOAD"){
            ...
            $filename = "calculator_analytics.csv";
            $handle = fopen("./storage/app/".$filename, 'w');
            fputcsv($handle, array('SN', 'Calculator Name', 'Type', 'User Name', 'User Email', 'D Type', 'Created Date', 'IP'));
            foreach($data['reports'] as $key=>$row) {
                $typee = "";
                if($row->list_count){
                    $typee = "List";
                ...
                fputcsv($handle, array($key+1, $row->page_name, $typee, $row->username, $row->user_email, $row->device_type,date('d-m-Y h:s', strtotime($row->created_at)), $row->ip));
            }
            fclose($handle);
            $headers = array(
                'Content-Type' => 'text/csv',
    ...
    public function mf_research(Request $request){
        ...
        if($data['page_type'] == "DOWNLOAD"){
            $pdf = PDF::loadView('admin.analytics.mf_research_pdf', $data);
            return $pdf->download('mf_research_analytics.pdf');
        }else if($data['page_type'] == "CSV"){
            $filename = "mf_research_analytics.csv";
            $handle = fopen("./storage/app/".$filename, 'w');
            fputcsv($handle, array('SN', 'MF Research Name', 'List Count', 'DownLoad Count', 'Save Count', 'Total Count'));
            foreach($data['reports'] as $key=>$row) {
                fputcsv($handle, array($key+1, $row->page_name, $row->total_list_count, $row->total_download_count, $row->total_save_count,$row->total_list_count+$row->total_download_count+$row->total_save_count));
            }
            fclose($handle);
            $headers = array(
                'Content-Type' => 'text/csv',
    ...
    public function mf_research_detail(Request $request){
        ...
        if($data['page_type'] == "DOWNLOAD"){
            ...
            $filename = "mf_research_analytics.csv";
            $handle = fopen("./storage/app/".$filename, 'w');
            fputcsv($handle, array('SN', 'MF Research Name', 'Type', 'User Name', 'User Email', 'User City', 'Package Name', 'D Type', 'Created Date', 'IP'));
            foreach($data['reports'] as $key=>$row) {
                $typee = "";
                if($row->list_count){
                    $typee = "List";
                ...
                fputcsv($handle, array($key+1, $row->page_name, $typee, $row->username, $row->user_email, $row->user_city, $row->package_name, $row->device_type,date('d-m-Y h:s', strtotime($row->created_at)), $row->ip));
            }
            fclose($handle);
            $headers = array(
                'Content-Type' => 'text/csv',
    ...
    public function mf_research_user_wise(Request $request){
        ...
        $data['reports'] = $data['reports']->where('page_type','=','MF Research')->whereNotNull('users.name')->groupBy(['user_id'])->get();
