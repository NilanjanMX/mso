...
class AnalyticsMembershipReferralController extends Controller
{
    ...
    public function membership_referral_list(Request $request){
        ...
        if($data['date_type']){
            ...
            $data['reports'] = $data['reports']->whereBetween('referral_links.created_at', [$from , $to]);
        }else{
            if($data['ustart_date'] && $data['uend_date']){
        ...
        if($data['page_type'] == "DOWNLOAD"){
            ...
            fputcsv($handle, array('SN', 'User Name', 'Referee Phone', 'Member Email', 'Created', 'IP'));
            
            foreach($data['reports'] as $key=>$row) {
                fputcsv($handle, array($key+1, $row->user_name, $row->phone_number, $row->email, date('d-m-Y h:i', strtotime($row->created_at)), $row->ip ));   
            }
            fclose($handle);
            $headers = array(
    ...
    public function membership_points_list(Request $request){
        ...
        if($data['date_type']){
            ...
            $data['reports'] = $data['reports']->whereBetween('referral_codes.created_at', [$from , $to]);
        }
        $data['reports'] = $data['reports']->get();
        ...
        if($data['page_type'] == "DOWNLOAD"){
            ...
            $handle = fopen("./storage/app/".$filename, 'w');
            fputcsv($handle, array('SN', 'Name', 'Amount', 'Note', 'Created'));
            
            foreach($data['reports'] as $key=>$row) {
                fputcsv($handle, array($key+1, $row->name, $row->referral_amount, $row->note, date('d-m-Y',strtotime($row->created_at))));   
            }
            fclose($handle);
            $headers = array(
                'Content-Type' => 'text/csv',
    ...
    public function coupon_code_list(Request $request){
        ...
        if($data['date_type']){
            ...
            $data['reports'] = $data['reports']->whereBetween('coupons.created_at', [$from , $to]);
        }else{
            if($data['ustart_date'] && $data['uend_date']){
                $date_from = explode('/', $data['ustart_date']);
                $date_to = explode('/', $data['uend_date']);
