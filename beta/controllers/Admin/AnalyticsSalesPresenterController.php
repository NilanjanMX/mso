...
class AnalyticsSalesPresenterController extends Controller
{
    ...
    public function name_of_member(Request $request){
        ...
        if($data['date_type']){
            ...
            $data['reports'] = $data['reports']->whereBetween('history_sales_presenters.created_at', [$from , $to]);
        }else{
            if($data['ustart_date'] && $data['uend_date']){
        ...
        if($data['page_type'] == "DOWNLOAD"){
            ...
            fputcsv($handle, array('SN', 'Member Name', 'Member Email', 'JPEG Count', 'PDF Count'));
            foreach($data['reports'] as $key=>$row) {
                fputcsv($handle, array($key+1, $row->user_name, $row->email, $row->jpeg_count, $row->pdf_count));
            }
            fclose($handle);
            $headers = array(
    ...
    public function most_download(Request $request){
        ...
        if($data['page_type'] == "DOWNLOAD"){
            ...
            $handle = fopen("./storage/app/".$filename, 'w');
            fputcsv($handle, array('SN', 'Sales Presenter Name', 'JPEG Count'));
    ...
    public function top_download(Request $request){
        ...
        $data['reports'] = $data['reports']->where('history_sales_presenters.is_jpeg',1)->orderBy('total_count','DESC')->groupBy(['salespresentersoftcopies.id'])->take(5)->get();
