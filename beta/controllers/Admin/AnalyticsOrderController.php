...
class AnalyticsOrderController extends Controller
{
    ...
    public function dashboard(Request $request){
        ...
        if($data['date_type']){
            ...
            $data['no_of_add_cart'] = HistoryOrder::where("value","=","ORDER CART")->where("page_type","=",'ORDER')->whereBetween('created_at', [$from, $to])->count();
            $data['no_of_checkout'] = HistoryOrder::where("value","=","CHECKOUT INITIATED")->where("page_type","=",'ORDER')->whereBetween('created_at', [$from, $to])->count();
            ...
            if($data['date_from'] && $data['date_to']){
                ...
                $data['no_of_add_cart'] = HistoryOrder::where("value","=","ORDER CART")->where("page_type","=",'ORDER')->count();
                $data['no_of_checkout'] = HistoryOrder::where("value","=","CHECKOUT INITIATED")->where("page_type","=",'ORDER')->count();
    ...
    public function added_to_cart(Request $request){
        ...
        if($data['date_type']){
            ...
            $data['reports'] = $data['reports']->whereBetween('history_orders.created_at', [$from , $to]);
        }else{
            if($data['ustart_date'] && $data['uend_date']){
        ...
        $data['reports'] = $data['reports']->where('page_type','=','ORDER')->where('value','=','ORDER CART')->whereNotNull("stationaries.title")->groupBy(['history_orders.page_id'])->orderBy('total_count','DESC')->get();
    ...
    public function least_product(Request $request){
        ...
        $data['reports'] = $data['reports']->where('page_type','=','ORDER')->where('value','=','ORDER CART')->whereNotNull("stationaries.title")->orderBy('total_count','ASC')->groupBy(['history_orders.page_id'])->get();
        ...
        if($data['page_type'] == "DOWNLOAD"){
            ...
            $handle = fopen("./storage/app/".$filename, 'w');
            fputcsv($handle, array('SN', 'Name', 'Amount', 'Total Count'));

            foreach($data['reports'] as $key=>$row) {
                fputcsv($handle, array($key+1, $row->user_name, $row->amount, $row->total_count));
            }
            fclose($handle);
    ...
    public function no_added_to_cart(Request $request){
        ...
        $data['reports'] = $data['reports']->where('page_type','=','ORDER')->where('value','=','ORDER CART')->whereNotNull("stationaries.title")->orderBy('total_count','DESC')->groupBy(['history_orders.page_id'])->get();
    ...
    public function revenue_generated(Request $request){
        ...
        if($data['date_type']){
            ...
            $data['reports'] = $data['reports']->whereBetween('orders.created_at', [$from , $to]);
        }else{
        ...
        if($data['page_type'] == "DOWNLOAD"){
            ...
            $handle = fopen("./storage/app/".$filename, 'w');
            fputcsv($handle, array('SN', 'Invoice ID', 'User Name', 'User City', 'Package Name', 'Total Amount', 'Date'));

            foreach($data['reports'] as $key=>$row) {
                fputcsv($handle, array($key+1, $row->invoice_id, $row->user_name, $row->user_city, $row->package_name, $row->payable_amount, date('d-m-Y h:s', strtotime($row->created_at))));
            }
            fclose($handle);
    ...
    public function point_used(Request $request){
        ...
        $reports = $data['reports']->where('payable_amount','!=',0)->orderBy('orders.created_at','DESC')->get();
    ...
    public function coupon_used(Request $request){
        ...
        $data['reports'] = $data['reports']->where('coupon_amount','!=',0)->orderBy('orders.created_at','DESC')->get();
    ...
    public function abandoned_checkouts(Request $request){
        ...
        if($data['date_type']){
            ...
            $data['reports'] = $data['reports']->whereBetween('user_carts.created_at', [$from , $to]);
        }else{
            if($data['ustart_date'] && $data['uend_date']){
                $date_from = explode('/', $data['ustart_date']);
                $date_to = explode('/', $data['uend_date']);
