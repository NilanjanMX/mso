...
class LimitedPeriodSipGoalPlanningCalculatorController extends Controller
{
    ...
    public function limitedPeriodSIPgoalPlanningCalculator(Request $request)
    {



        if($request->action == "back"){
            if (session()->has('limited_period_sip_goal_planning_calculator_form_data_edit')){
                session()->forget('limited_period_sip_goal_planning_calculator_form_data_edit');
            }
            
            if (Session::has('limited_period_sip_goal_planning_calculator_form_data')) {
                $saveCalculatorsData = Session::get('limited_period_sip_goal_planning_calculator_form_data');
                // dd($saveCalculatorsData['client']);
                $data = [];
                $data['form_data'] = $saveCalculatorsData;
                ...
                $data['calculater_heading'] = CalculatorHeading::where('key_name','=','limited_period_sip_goal_planning_calculator')->first();
                // dd($data);

                $data['details'] = DB::table("calculators")->where("url","calculators/sip-calculator/limited-period-sip-need-based-calculator")->first();
                // dd($data);
                return view('frontend.calculators.limitedperiodsipgoalplanning.limited_period_sip_goal_planning_calculator_edit',$data);
                
            }else{
                return redirect()->route('frontend.limitedPeriodSIPgoalPlanningCalculator');
            }
        }else{
            ...
            if (session()->has('limited_period_sip_goal_planning_calculator_form_data')){
                session()->forget('limited_period_sip_goal_planning_calculator_form_data');
            }
            if (session()->has('limited_period_sip_goal_planning_calculator_form_data_edit')){
                session()->forget('limited_period_sip_goal_planning_calculator_form_data_edit');
            }
            ...
            $data['details'] = DB::table("calculators")->where('url','calculators/sip-calculator/limited-period-sip-need-based-calculator')->first();
            // dd($data);
            return view('frontend.calculators.limitedperiodsipgoalplanning.limited_period_sip_goal_planning_calculator',$data);
        }
    }

    public function limitedPeriodSIPgoalPlanningCalculatorOutput(Request $request)
    {
        if ($request->isMethod('get')) {
            if (Session::has('limited_period_sip_goal_planning_calculator_form_data')) {
                $input = Session::get('limited_period_sip_goal_planning_calculator_form_data');
        ...
        if ($request->isMethod('post')) {
            $request->validate([
                ...
                'deferment_period' => 'required|numeric|between:1,99',
            ...
            Session::put('limited_period_sip_goal_planning_calculator_form_data', $data);
        }
        ...
        $category_detail = Calculator::where("url", "calculators/sip-calculator/limited-period-sip-need-based-calculator")->first();
        ...
        $edit_id = session()->get('limited_period_sip_goal_planning_calculator_form_data_edit');
        if(!empty($edit_id)){
            $calc = SaveCalculators::where('user_id',Auth::user()->id)->where('id',$edit_id)->first();
            $data['cal_name'] = $calc->title;
        }

        $calculators = DB::table("calculators")->where("id", 8)->first();
        if ($calculators) {
        ...
        $data['edit_id'] = session()->get('limited_period_sip_goal_planning_calculator_form_data_edit');
        return view('frontend.calculators.limitedperiodsipgoalplanning.limited_period_sip_goal_planning_calculator_output', $data);
    }

    public function limitedPeriodSIPgoalPlanningCalculatorOutputSave(Request $request)
    {
        ...
        if ($request->isMethod('get')) {
            if (Session::has('limited_period_sip_goal_planning_calculator_form_data')) {
                ...
                $view = (string) View::make('frontend.calculators.limitedperiodsipgoalplanning.limited_period_sip_goal_planning_calculator_output_pdf', $data);
                $edit_id = session()->get('limited_period_sip_goal_planning_calculator_form_data_edit');
                if ($edit_id) {
    ...
    public function limitedPeriodSIPgoalPlanningCalculatorEdit(Request $request)
    {
        ...
        $data['form_data']['sip_period'] = $saveCalculatorsData['sip_period'];
        $data['form_data']['interest1'] = $saveCalculatorsData['interest1'];
        if (isset($saveCalculatorsData['interest2'])) {
        ...
        if (isset($saveCalculatorsData['is_note'])) {
            $data['form_data']['is_note'] = $saveCalculatorsData['is_note'];
        }
        if (isset($saveCalculatorsData['is_graph'])) {
        ...
        Session::put('limited_period_sip_goal_planning_calculator_form_data_edit', $request->id);
    ...
    public function merge_download(Request $request)
    {
        ...
        $data['title'] = "Limited Period SIP Goal";
        ...
        if ($data['before_after'] == 'after') {
            $pdf = PDF::loadView('frontend.calculators.limitedperiodsipgoalplanning.limited_period_sip_goal_planning_calculator_output_pdf', $data);
        ...
        if($data['saved_sp_list_id']){
            foreach ($data['saved_sp_list_id'] as $key => $value) {
                ...
                if (isset($softcopieslists) && count($softcopieslists) > 0) {
                    ...
                    // dd($data1);
                    $pdf = PDF::loadView('frontend.calculators.limitedperiodsipgoalplanning.limited_period_sip_goal_planning_calculator_output_pdf', $data1);
    ...
    public function limitedPeriodSIPgoalPlanningCalculatorView(Request $request)
    {
        ...
        $data['details'] = DB::table("calculators")->where('url', 'calculators/sip-calculator/limited-period-sip-need-based-calculator')->first();
        return view('frontend.calculators.limitedperiodsipgoalplanning.limited_period_sip_goal_planning_calculator_view', $data);
    }

    public function limitedPeriodSIPgoalPlanningCalculatorOutputPdfDownload(Request $request)
    {
        if ($request->isMethod('get')) {
            if (Session::has('limited_period_sip_goal_planning_calculator_form_data')) {
                $data = Session::get('limited_period_sip_goal_planning_calculator_form_data');
                ...
                $data['title'] = "Limited Period SIP Report";
