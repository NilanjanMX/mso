...
class FutureValueLumpsumSipController extends Controller
{
    ...
    public function index(){
        $data['activemenu'] = 'combination';
        return view('frontend.calculators.futurevaluelumpsumsip.index',$data);
    }

    //Future Value Of Lumpsum + SIP
    public function futureValueOfLumpsumSip(Request $request){
        if($request->action == "back"){
            if (session()->has('future_value_of_lumpsum_sip_form_id')){
                session()->forget('future_value_of_lumpsum_sip_form_id');
            }
            if (Session::has('future_value_of_lumpsum_sip_form_data')) {
                $saveCalculatorsData = Session::get('future_value_of_lumpsum_sip_form_data');
                ...
                $data['form_data']['expected_rate_of_return1'] = isset($saveCalculatorsData['expected_rate_of_return1'])?$saveCalculatorsData['expected_rate_of_return1']:'';
                $data['form_data']['lumpsum_investment'] = isset($saveCalculatorsData['lumpsum_investment'])?$saveCalculatorsData['lumpsum_investment']:'';
                $data['form_data']['investment_period'] = isset($saveCalculatorsData['investment_period'])?$saveCalculatorsData['investment_period']:'';
                $data['form_data']['expected_rate_of_return2'] = isset($saveCalculatorsData['expected_rate_of_return2'])?$saveCalculatorsData['expected_rate_of_return2']:'';
                ...
                $data['calculater_heading'] = CalculatorHeading::where('key_name','=','future_value_of_lumpsum_+_sip')->first();
                // dd($data);

                $data['details'] = DB::table("calculators")->where("url","calculators/combination/future-value-of-lumpsum-sip")->first();
                // dd($data);
                return view('frontend.calculators.futurevaluelumpsumsip.edit',$data);
                
            }else{
                return redirect()->route('frontend.futureValueOfLumpsumSip');
            }
        }else{
            ...
            if (session()->has('future_value_of_lumpsum_sip_form_data')){
                session()->forget('future_value_of_lumpsum_sip_form_data');
            }
            if (session()->has('future_value_of_lumpsum_sip_form_id')){
                session()->forget('future_value_of_lumpsum_sip_form_id');
            }
            ...
            $data['details'] = DB::table("calculators")->where('url','calculators/combination/future-value-of-lumpsum-sip')->first();
            // dd($data);
            return view('frontend.calculators.futurevaluelumpsumsip.future_value_of_lumpsum_sip',$data);
        }
    }

    public function futureValueOfLumpsumSipOutput(Request $request){
        if ($request->isMethod('get')) {
            if (Session::has('future_value_of_lumpsum_sip_form_data')) {
                $input = Session::get('future_value_of_lumpsum_sip_form_data');
                $data['sip_amount'] = $input['sip_amount'];
                $data['expected_rate_of_return1'] = $input['expected_rate_of_return1'];
                $data['lumpsum_investment'] = $input['lumpsum_investment'];
                $data['investment_period'] = $input['investment_period'];
                $data['report'] = $input['report'];

                if (isset($input['expected_rate_of_return2']) && $input['expected_rate_of_return2'] != '') {
                    $data['expected_rate_of_return2'] = $input['expected_rate_of_return2'];
                }else{
                    $data['expected_rate_of_return2'] = 0;
                }
        ...
        if ($request->isMethod('post')) {
            $request->validate([
                'sip_amount' => 'required|numeric|between:100,99999999',
                'expected_rate_of_return1' => 'required|numeric|between:0.1,18',
                'lumpsum_investment' => 'required|numeric|between:100,9999999999',
                'expected_rate_of_return2' => 'sometimes|nullable|numeric|between:0.1,18',
            ...
            if ($input['expected_rate_of_return2'] != '') {
                $data['expected_rate_of_return2'] = $input['expected_rate_of_return2'];
            }else{
                $data['expected_rate_of_return2'] = 0;
            }
            ...
            Session::put('future_value_of_lumpsum_sip_form_data', $data);
        }
        ...
        $category_detail = Calculator::where("url","calculators/combination/future-value-of-lumpsum-sip")->first();
        ...
        $edit_id = session()->get('future_value_of_lumpsum_sip_form_id');
        if(!empty($edit_id)){
            $calc = SaveCalculators::where('user_id',Auth::user()->id)->where('id',$edit_id)->first();
            $data['cal_name'] = $calc->title;
        }
        
        $calculators = DB::table("calculators")->where("id",23)->first();
        if($calculators){
        ...
        $data['edit_id'] = session()->get('future_value_of_lumpsum_sip_form_id');

        return view('frontend.calculators.futurevaluelumpsumsip.future_value_of_lumpsum_sip_output',$data);
    }

    public function futureValueOfLumpsumSipOutputSave(Request $request){
        ...
        if ($request->isMethod('get')) {
            if (Session::has('future_value_of_lumpsum_sip_form_data')) {
                ...
                $view = (string)View::make('frontend.calculators.futurevaluelumpsumsip.future_value_of_lumpsum_sip_output_pdf',$data);
                $edit_id = session()->get('future_value_of_lumpsum_sip_form_id');
                if($edit_id){
    ...
    public function futureValueOfLumpsumSipOutputPdfDownload(Request $request){
        if ($request->isMethod('get')) {
            if (Session::has('future_value_of_lumpsum_sip_form_data')) {
                $input = Session::get('future_value_of_lumpsum_sip_form_data');
                $data['sip_amount'] = $input['sip_amount'];
                ...
                $data['title'] = "Future Value Of Lumpsum + SIP";
                ...
                if($data['pdf_title_line1']){
                    ...
                    $pdf = PDF::loadView('frontend.calculators.futurevaluelumpsumsip.future_value_of_lumpsum_sip_output_pdf', $data);
    ...
    public function edit(Request $request){
        ...
        Session::put('future_value_of_lumpsum_sip_form_id', $request->id);
    ...
    public function view(Request $request){
        ...
        $data['details'] = DB::table("calculators")->where('url','calculators/combination/future-value-of-lumpsum-sip')->first();
        return view('frontend.calculators.futurevaluelumpsumsip.view',$data);
    }
        ...
        if($data['saved_sp_list_id']){
            foreach ($data['saved_sp_list_id'] as $key => $value) {
                ...
                if (isset($softcopieslists) && count($softcopieslists)>0){
                    ...
                    // dd($data1);
                    $pdf = PDF::loadView('frontend.calculators.futurevaluelumpsumsip.future_value_of_lumpsum_sip_output_pdf', $data1);
