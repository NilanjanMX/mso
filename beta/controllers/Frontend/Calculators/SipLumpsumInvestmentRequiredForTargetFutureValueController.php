...
class SipLumpsumInvestmentRequiredForTargetFutureValueController extends Controller
{
    ...
    public function sipLumpsumInvestmentTargetFutureValue(Request $request){
        
        if($request->action == "back"){
            if (session()->has('sip_lumpsum_investment_required_target_value_id')){
                session()->forget('sip_lumpsum_investment_required_target_value_id');
            }
            // dd(Session::get('sip_lumpsum_investment_required_target_value_data'));
            if (Session::has('sip_lumpsum_investment_required_target_value_data')) {
                $saveCalculatorsData = Session::get('sip_lumpsum_investment_required_target_value_data');
                ...
                $data['form_data']['target_amount'] = $saveCalculatorsData['target_amount'];
                $data['form_data']['investment_period'] = $saveCalculatorsData['investment_period'];
                $data['form_data']['investment_type'] = $saveCalculatorsData['investment_type'];
                $data['form_data']['investment_amount'] = $saveCalculatorsData['investment_amount'];
                $data['form_data']['onetime_interest_rate'] = $saveCalculatorsData['onetime_interest_rate'];
                $data['form_data']['sip_interest_rate'] = $saveCalculatorsData['sip_interest_rate'];
                ...
                $data['calculater_heading'] = CalculatorHeading::where('key_name','=','sip_lumpsum_investment_required_for_target_future_value')->first();
                // dd($data);

                $data['details'] = DB::table("calculators")->where("url","calculators/combination/sip-or-lumpsum-investment-required-for-target-future-value")->first();
                // dd($data);
                return view('frontend.calculators.siplumpsuminvestmentrequiredfortargetfuturevalue.edit',$data);
                
            }else{
                return redirect()->route('frontend.sipLumpsumInvestmentTargetFutureValue');
            }
        }else{
            ...
            if (session()->has('sip_lumpsum_investment_required_target_value_data')){
                session()->forget('sip_lumpsum_investment_required_target_value_data');
            }
            if (session()->has('sip_lumpsum_investment_required_target_value_id')){
                session()->forget('sip_lumpsum_investment_required_target_value_id');
            }
            ...
            $data['details'] = DB::table("calculators")->where('url','calculators/combination/sip-or-lumpsum-investment-required-for-target-future-value')->first();
            // dd($data);
            return view('frontend.calculators.siplumpsuminvestmentrequiredfortargetfuturevalue.index',$data);
        }
    }

    public function sipLumpsumInvestmentTargetFutureValueOutput(Request $request){
        if ($request->isMethod('get')) {
            if (Session::has('sip_lumpsum_investment_required_target_value_data')) {
                $input = Session::get('sip_lumpsum_investment_required_target_value_data');
                // dd($input);
                $input = Session::get('sip_lumpsum_investment_required_target_value_data');
                $data['target_amount'] = $input['target_amount'];
                $data['investment_period'] = $input['investment_period'];
                $data['investment_type'] = $input['investment_type'];
                $data['investment_amount'] = $input['investment_amount'];
                $data['onetime_interest_rate'] = $input['onetime_interest_rate'];
                $data['sip_interest_rate'] = $input['sip_interest_rate'];
        ...
        if ($request->isMethod('post')) {
            if ($request->initial_investment>100) {
                $initial_investment = round($request->initial_investment*2/100);
            }
            // $request->validate([
            //     'initial_investment' => 'required|numeric|between:100,9999999999',
            //     'investment_period' => 'required|numeric|between:1,99',
            //     'debt_fund' => 'required|numeric|between:0.1,18',
            //     'equity_fund' => 'required|numeric|between:0.1,18',
            //    // 'step_up_rate' => 'sometimes|nullable|numeric|between:1,25|different:interest1|different:interest2',
            //     'clientname' => 'sometimes|nullable|max:30',
            //     'fixed_percent' => 'sometimes|nullable|numeric|between:0.1,2.0',
            //     'fixed_amount' => "sometimes|nullable|numeric|between:100,$initial_investment,",
            // ]);
            ...
            Session::put('sip_lumpsum_investment_required_target_value_data', $data);
        }
        ...
        $category_detail = Calculator::where("url","calculators/combination/sip-or-lumpsum-investment-required-for-target-future-value")->first();
        ...
        $edit_id = session()->get('sip_lumpsum_investment_required_target_value_id');
        if(!empty($edit_id)){
            $calc = SaveCalculators::where('user_id',Auth::user()->id)->where('id',$edit_id)->first();
            $data['cal_name'] = $calc->title;
        }
        
        $calculators = DB::table("calculators")->where("id",24)->first();
        if($calculators){
        ...
        $data['edit_id'] = session()->get('sip_lumpsum_investment_required_target_value_id');
        // dd(session()->get('sip_lumpsum_investment_required_target_value_id'));
        return view('frontend.calculators.siplumpsuminvestmentrequiredfortargetfuturevalue.output',$data);
    }

    public function sipLumpsumInvestmentTargetFutureValueOutputSave(Request $request){
        ...
        if ($request->isMethod('get')) {
            if (Session::has('sip_lumpsum_investment_required_target_value_data')) {
                ...
                $view = (string)View::make('frontend.calculators.siplumpsuminvestmentrequiredfortargetfuturevalue.output_pdf',$data);
    ...
    public function sipLumpsumInvestmentTargetFutureValueOutputPdfDownload(Request $request){
        if ($request->isMethod('get')) {
            if (Session::has('sip_lumpsum_investment_required_target_value_data')) {
                $input = Session::get('sip_lumpsum_investment_required_target_value_data');
                $data['target_amount'] = $input['target_amount'];
                ...
                $data['title'] = "SIP / Lumpsum Required For Target Future Value";
                ...
                if($data['pdf_title_line1']){
                    ...
                    $pdf = PDF::loadView('frontend.calculators.siplumpsuminvestmentrequiredfortargetfuturevalue.output_pdf', $data);
    ...
    public function edit(Request $request){
        ...
        Session::put('sip_lumpsum_investment_required_target_value_id', $request->id);
    ...
    public function view(Request $request){
        ...
        $data['details'] = DB::table("calculators")->where('url','calculators/combination/sip-or-lumpsum-investment-required-for-target-future-value')->first();
        // dd($data);
        return view('frontend.calculators.siplumpsuminvestmentrequiredfortargetfuturevalue.view',$data);
    }
