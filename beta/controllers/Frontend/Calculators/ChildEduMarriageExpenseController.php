...
class ChildEduMarriageExpenseController extends Controller
{
    ...
    public function index(Request $request)
    {
        if($request->action == "back"){
            if (session()->has('child_edu_marriage_expenses_form_id')){
                session()->forget('child_edu_marriage_expenses_form_id');
            }
            // dd(Session::get('childEducation_form_data'));
            if (Session::has('childEducation_form_data')) {
                $saveCalculatorsData = Session::get('childEducation_form_data');
                
                // dd($saveCalculatorsData);
                
                // if (session()->has('childEducation_form_data')){
                //     session()->forget('childEducation_form_data');
                // }
                ...
                $data['calculater_heading'] = CalculatorHeading::where('key_name','=','child_education_marriage_planning')->first();
                // dd($data);

                $data['details'] = DB::table("calculators")->where("url","calculators/need-based-calculator/child-education-need-based-calculator")->first();
                // dd($data);
                return view('frontend.calculators.child_edu_marriage_expenses.edit',$data);
                
            }else{
                return redirect()->route('frontend.childEducation');
            }
        }else{
            ...
            $data = [];
            $data['calculater_heading'] = CalculatorHeading::where('key_name','=','child_education_marriage_planning')->first();
            return view('frontend.calculators.child_edu_marriage_expenses.index',$data); 
    ...
    public function output(Request $request)
    {
        if ($request->isMethod('get')) {
            if (Session::has('childEducation_form_data')) {
                $input = Session::get('childEducation_form_data');
        ...
        if ($request->isMethod('post')) {
            ...
            $request->validate([
                'child_name' => 'required|nullable|regex:/^[a-zA-Z ]*$/|max:30',
                'current_age' => 'required|numeric|between:0,99',
                'fund_required_age' => 'required|numeric|gt:current_age|between:1,99',
                'fund_required_amount' => 'required|numeric|between:100,9999999999',
                'inflation_rate' => 'required|numeric|between:0,15',
                'return_rate_1' => 'required|numeric|between:0.1,18',
                'return_rate_2' => 'sometimes|nullable|numeric|between:0.1,18',
            ...
            Session::put('childEducation_form_data', $data);
        }
        ...
        $category_detail = Calculator::where("url","calculators/need-based-calculator/child-education-need-based-calculator")->first();
        ...
        $edit_id = session()->get('child_edu_marriage_expenses_form_id');
        if(!empty($edit_id)){
            $calc = SaveCalculators::where('user_id',Auth::user()->id)->where('id',$edit_id)->first();
            $data['cal_name'] = $calc->title;
        }
        
        $calculators = DB::table("calculators")->where("id",27)->first();
        if($calculators){
        ...
        $data['edit_id'] = session()->get('child_edu_marriage_expenses_form_id');

        return view('frontend.calculators.child_edu_marriage_expenses.output',$data);
    }

    public function pdf(Request $request){
        if ($request->isMethod('get')) {
            if (Session::has('childEducation_form_data')) {
                $data = Session::get('childEducation_form_data');
                ...
                if($data['pdf_title_line1']){
                    ...
                    $pdf = PDF::loadView('frontend.calculators.child_edu_marriage_expenses.pdf', $data);
    ...
    public function save(Request $request){
        ...
        if ($request->isMethod('get')) {
            if (Session::has('childEducation_form_data')) {
                ...
                $view = (string)View::make('frontend.calculators.child_edu_marriage_expenses.pdf',$data);
                $edit_id = session()->get('child_edu_marriage_expenses_form_id');
    ...
    public function edit(Request $request){
        ...
        Session::put('child_edu_marriage_expenses_form_id', $request->id);
    ...
    public function view(Request $request){
        ...
        $data['details'] = DB::table("calculators")->where('url','calculators/need-based-calculator/child-education-need-based-calculator')->first();
        return view('frontend.calculators.child_edu_marriage_expenses.view',$data);
    }
