...
class FutureValueOfAnnualLumpSumInvestmentController extends Controller
{
    ...
    public function index(Request $request)
    {
        if($request->action == "back"){
            if (session()->has('future_value_of_annual_lump_sum_investment_form_id')){
                session()->forget('future_value_of_annual_lump_sum_investment_form_id');
            }
            // dd(Session::get('future_value_of_annual_lump_sum_investment_form_data'));
            if (Session::has('future_value_of_annual_lump_sum_investment_form_data')) {
                $saveCalculatorsData = Session::get('future_value_of_annual_lump_sum_investment_form_data');
                
                // dd($saveCalculatorsData);
                
                // if (session()->has('future_value_of_annual_lump_sum_investment_form_data')){
                //     session()->forget('future_value_of_annual_lump_sum_investment_form_data');
                // }
                ...
                $data['calculater_heading'] = CalculatorHeading::where('key_name','=','future_value_of_annual_lump_sum_investment')->first();
                // dd($data);

                $data['details'] = DB::table("calculators")->where("url","calculators/onetime-investment/future-value-of-annual-lumpsum-investment")->first();
                // dd($data);
                return view('frontend.calculators.future_value_of_annual_lump_sum_investment.edit',$data);
                
            }else{
                return redirect()->route('frontend.futureValueOfAnnualLumpsumInvestment');
            }
        }else{
            ...
            $data = [];
            $data['calculater_heading'] = CalculatorHeading::where('key_name','=','future_value_of_annual_lump_sum_investment')->first();
            return view('frontend.calculators.future_value_of_annual_lump_sum_investment.index',$data); 
    ...
    public function output(Request $request)
    {
        if ($request->isMethod('get')) {
            if (Session::has('future_value_of_annual_lump_sum_investment_form_data')) {
                $input = Session::get('future_value_of_annual_lump_sum_investment_form_data');
                
                $data['loan_amount'] = $input['loan_amount'];
                $data['investment_period'] = $input['investment_period'];
                $data['payment_period'] = $input['payment_period'];
        ...
        if ($request->isMethod('post')) {
            ...
            $request->validate([
                ...
                'payment_period' => 'required|numeric|between:1,99',
            ...
            Session::put('future_value_of_annual_lump_sum_investment_form_data', $data);
        }
        ...
        $category_detail = Calculator::where("url","calculators/onetime-investment/future-value-of-annual-lumpsum-investment")->first();
        ...
        $edit_id = session()->get('future_value_of_annual_lump_sum_investment_form_id');
        if(!empty($edit_id)){
            $calc = SaveCalculators::where('user_id',Auth::user()->id)->where('id',$edit_id)->first();
            $data['cal_name'] = $calc->title;
        }
        
        $calculators = DB::table("calculators")->where("id",49)->first();
        if($calculators){
        ...
        $data['edit_id'] = session()->get('future_value_of_annual_lump_sum_investment_form_id');

        return view('frontend.calculators.future_value_of_annual_lump_sum_investment.output',$data);
    }

    public function pdf(Request $request){
        if ($request->isMethod('get')) {
            if (Session::has('future_value_of_annual_lump_sum_investment_form_data')) {
                $data = Session::get('future_value_of_annual_lump_sum_investment_form_data');
                ...
                $data['title'] = "Child Education Need Based Calculator";
                ...
                if($data['pdf_title_line1']){
                    ...
                    $pdf = PDF::loadView('frontend.calculators.future_value_of_annual_lump_sum_investment.pdf', $data);
                    ...
                    // $html = view('frontend.calculators.future_value_of_annual_lump_sum_investment.pdf', $data)->render();
                    // var_dump($html);exit;
                    $pdf = PDF::loadView('frontend.calculators.future_value_of_annual_lump_sum_investment.pdf', $data);
                ...
                // $pdf = PDF::loadView('frontend.calculators.future_value_of_annual_lump_sum_investment.pdf', $data);
                // return $pdf->download(time().'_recover_emis_through_sip.pdf');
    ...
    public function save(Request $request){
        ...
        if ($request->isMethod('get')) {
            if (Session::has('future_value_of_annual_lump_sum_investment_form_data')) {
                ...
                $view = (string)View::make('frontend.calculators.future_value_of_annual_lump_sum_investment.pdf',$data);
                $edit_id = session()->get('future_value_of_annual_lump_sum_investment_form_id');
    ...
    public function edit(Request $request){
        ...
        Session::put('future_value_of_annual_lump_sum_investment_form_id', $request->id);
    ...
    public function view(Request $request){
        ...
        $data['edit_id'] = 0;
        $data['id'] = $request->id;
        $calculators = DB::table("calculators")->where("id",1)->first();
        if($calculators){
            $package_id = Auth::user()->package_id;
            $permission = DB::table("calculator_permissions")->where("calculator_id",$calculators->id)->where("package_id",$package_id)->first();
            if($permission){
        ...
        $data['details'] = DB::table("calculators")->where('url','calculators/onetime-investment/future-value-of-annual-lumpsum-investment')->first();
        return view('frontend.calculators.future_value_of_annual_lump_sum_investment.view',$data);
    }
    ...
    public function merge_download(Request $request){
        ...
        $data['title'] = "Annual Lumsump Calculator";
