...
class CapitalGainsTaxCalcController extends Controller
{

    //Lumpsum Investment Ready Reckoner
    public function index(Request $request){
        
        if($request->action == "back"){
            if (session()->has('capital_gains_tax_calculator_form_id')){
                session()->forget('capital_gains_tax_calculator_form_id');
            }
            // dd(Session::get('capital_gains_tax_calculator_form_data'));
            if (Session::has('capital_gains_tax_calculator_form_data')) {
                $saveCalculatorsData = Session::get('capital_gains_tax_calculator_form_data');
                ...
                $data['financial_years'] = CapitalGainsCalculator::orderBy('index','asc')->get();
                foreach($data['financial_years'] as $years){
                    $check_fy[] = $years->financial_year;
                }
                $data['check_fy'] = $check_fy;
        
                $data['calculater_heading'] = CalculatorHeading::where('key_name','=','capital_gains_tax_calculator')->first();
                // dd($data);

                $data['details'] = DB::table("calculators")->where("url","premium-calculator/capital_gains_tax_calculator")->first();
                // dd($data);
                return view('frontend.calculators.capital_gains_tax_calculator.edit',$data);
                
            }else{
                return redirect()->route('frontend.capital_gains_tax_calculator');
            }
        }else{
            ...
            if (session()->has('capital_gains_tax_calculator_form_data')){
                session()->forget('capital_gains_tax_calculator_form_data');
            }
            if (session()->has('capital_gains_tax_calculator_form_id')){
                session()->forget('capital_gains_tax_calculator_form_id');
            }
            ...
            $data['details'] = DB::table("calculators")->where('url','premium-calculator/capital_gains_tax_calculator')->first();
            // dd($data);
            return view('frontend.calculators.capital_gains_tax_calculator.index',$data);
        }
    }
    ...
    public function output(Request $request){
        if ($request->isMethod('get')) {
            if (Session::has('capital_gains_tax_calculator_form_data')) {
                $input = Session::get('capital_gains_tax_calculator_form_data');
                // dd($input);
                $data = $input;
        ...
        if ($request->isMethod('post')) {
            ...
            $input = $request->all();
            $input['financial_year'] = isset($input['financial_year'])?$input['financial_year']:[0];
            $input['cost_of_improvement'] = isset($input['cost_of_improvement'])?$input['cost_of_improvement']:[0];
            $input['sales_expenses'] = isset($input['sales_expenses'])?$input['sales_expenses']:0;
            $input['income_tax_slab'] = isset($input['income_tax_slab'])?$input['income_tax_slab']:'';
            
            if($input['purchased_before'] == 1){
                $request->validate([
                    'fair_market_value' => 'required|numeric|between:100,9999999999',
                    'sales_price' => 'required|numeric|between:100,9999999999',
                ]);
            }else{
                $request->validate([
                    'sales_price' => 'required|numeric|between:100,9999999999',
                    'purchase_price' => 'required|numeric|between:100,9999999999',
                ]);
            }

            $asset_type = isset($input['asset_type'])?$input['asset_type']:'movable';
            $rates = CapitalGainsTaxRate::where('asset_class', $asset_type)->where('term', 'short_term')->first();
            $input['short_term_tax'] = $rates->rate;
            $rates = CapitalGainsTaxRate::where('asset_class', $asset_type)->where('term', 'long_term')->first();
            $input['long_term_tax'] = $rates->rate;

            $financial_years = CapitalGainsCalculator::orderBy('index','asc')->get();
            foreach($financial_years as $financial_year){
                $fy_index[$financial_year->financial_year] = $financial_year->index;
            }
            $input['financial_years'] = $fy_index;
            $data = $input;
            ...
            Session::put('capital_gains_tax_calculator_form_data', $data);
        }
        ...
        $category_detail = Calculator::where("url","premium-calculator/capital_gains_tax_calculator")->first();
        ...
        $edit_id = session()->get('capital_gains_tax_calculator_form_id');
        if(!empty($edit_id)){
            $calc = SaveCalculators::where('user_id',Auth::user()->id)->where('id',$edit_id)->first();
            $data['cal_name'] = $calc->title;
        }
        
        $calculators = DB::table("calculators")->where("id",42)->first();
        if($calculators){
        ...
        $data['edit_id'] = session()->get('capital_gains_tax_calculator_form_id');
        // dd($data);
        return view('frontend.calculators.capital_gains_tax_calculator.output',$data);
    }
    ...
    public function save(Request $request){
        ...
        if ($request->isMethod('get')) {
            if (Session::has('capital_gains_tax_calculator_form_data')) {
                ...
                $view = (string)View::make('frontend.calculators.capital_gains_tax_calculator.pdf',$data);
        ...
        if ($request->isMethod('get')) {
            if (Session::has('capital_gains_tax_calculator_form_data')) {
                ...
                if($data['pdf_title_line1']){
                    ...
                    $pdf = PDF::loadView('frontend.calculators.capital_gains_tax_calculator.pdf', $data);
    ...
    public function edit(Request $request){
        ...
        Session::put('capital_gains_tax_calculator_form_id', $request->id);
    ...
    public function view(Request $request){
        ...
        $data['details'] = DB::table("calculators")->where('url','premium-calculator/capital_gains_tax_calculator')->first();
        return view('frontend.calculators.capital_gains_tax_calculator.view',$data);
    }
    ...
    public function merge_download(Request $request){
        ...
        $data['title'] = "Capital Gains Tax Calculation";
