...
class FutureValueLimitedPeriodSIPController extends Controller
{
    ...
    public function index(Request $request)
    {
        if($request->action == "back"){
            if (session()->has('limited_period_sip_form_id')){
                session()->forget('limited_period_sip_form_id');
            }
            // dd(Session::get('limited_period_sip_form_data'));
            if (Session::has('limited_period_sip_form_data')) {
                $saveCalculatorsData = Session::get('limited_period_sip_form_data');
                
                // dd($saveCalculatorsData);
                
                // if (session()->has('limited_period_sip_form_data')){
                //     session()->forget('limited_period_sip_form_data');
                // }
                ...
                $data['calculater_heading'] = CalculatorHeading::where('key_name','=','future_value_of_limited_period_sip')->first();
                // dd($data);

                $data['details'] = DB::table("calculators")->where("url","calculators/sip-calculator/limited-period-sip-calculator")->first();
                // dd($data);
                return view('frontend.calculators.future_value_of_limited_period_sip.edit',$data);
                
            }else{
                return redirect()->route('frontend.limitedPeriodSIPfutureValueAfterDefermentPeriod');
            }
        }else{
            ...
            $data = [];
            $data['calculater_heading'] = CalculatorHeading::where('key_name','=','future_value_of_limited_period_sip')->first();
            return view('frontend.calculators.future_value_of_limited_period_sip.index',$data); 
    ...
    public function output(Request $request)
    {
        if ($request->isMethod('get')) {
            if (Session::has('limited_period_sip_form_data')) {
                $input = Session::get('limited_period_sip_form_data');
        ...
        if ($request->isMethod('post')) {
            ...
            Session::put('limited_period_sip_form_data', $data);
        }
        ...
        $category_detail = Calculator::where("url","calculators/sip-calculator/limited-period-sip-calculator")->first();
        ...
        $edit_id = session()->get('limited_period_sip_form_id');
        if(!empty($edit_id)){
            $calc = SaveCalculators::where('user_id',Auth::user()->id)->where('id',$edit_id)->first();
            $data['cal_name'] = $calc->title;
        }
        
        $calculators = DB::table("calculators")->where("id",7)->first();
        if($calculators){
        ...
        $data['edit_id'] = session()->get('limited_period_sip_form_id');

        return view('frontend.calculators.future_value_of_limited_period_sip.output',$data);
    }

    public function pdf(Request $request){
        if ($request->isMethod('get')) {
            if (Session::has('limited_period_sip_form_data')) {
                $data = Session::get('limited_period_sip_form_data');
                ...
                if($data['pdf_title_line1']){
                    ...
                    $pdf = PDF::loadView('frontend.calculators.future_value_of_limited_period_sip.pdf', $data);
    ...
    public function save(Request $request){
        ...
        if ($request->isMethod('get')) {
            if (Session::has('limited_period_sip_form_data')) {
                ...
                $view = (string)View::make('frontend.calculators.future_value_of_limited_period_sip.pdf',$data);
                $edit_id = session()->get('limited_period_sip_form_id');
    ...
    public function edit(Request $request){
        ...
        Session::put('limited_period_sip_form_id', $request->id);
    ...
    public function view(Request $request){
        ...
        $data['details'] = DB::table("calculators")->where('url','calculators/sip-calculator/limited-period-sip-calculator')->first();
        return view('frontend.calculators.future_value_of_limited_period_sip.view',$data);
    }
