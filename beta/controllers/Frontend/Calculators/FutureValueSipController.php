...
class FutureValueSipController extends Controller
{
    ...
    public function futureValueOfSipIndex(Request $request)
    {

        if ($request->action == "back") {
            if (session()->has('future_value_of_sip_new')) {
                session()->forget('future_value_of_sip_new');
            }

            if (Session::has('future_value_of_sip_form_data')) {
                $saveCalculatorsData = Session::get('future_value_of_sip_form_data');
                ...
                $data['form_data']['step_up_rate'] = isset($saveCalculatorsData['step_up_rate']) ? $saveCalculatorsData['step_up_rate'] : "";
                $data['form_data']['step_up_amount'] = isset($saveCalculatorsData['step_up_amount']) ? $saveCalculatorsData['step_up_amount'] : "";
                $data['form_data']['include_step_up'] = $saveCalculatorsData['include_step_up'];
                ...
                $data['calculater_heading'] = CalculatorHeading::where('key_name', '=', 'future_value_of_sip')->first();

                $data['details'] = DB::table("calculators")->where('url', 'calculators/sip-calculator/future-value-of-sip')->first();
                
                return view('frontend.calculators.futurevalueofsip.edit_sip', $data);

            } else {
                return redirect()->route('frontend.futureValueOfSipIndex');
            }
        } else {
            ...
            if (session()->has('future_value_of_sip_form_data')) {
                session()->forget('future_value_of_sip_form_data');
            }
            if (session()->has('future_value_of_sip_new')) {
                session()->forget('future_value_of_sip_new');
            }
            ...
            $data['details'] = DB::table("calculators")->where('url', 'calculators/sip-calculator/future-value-of-sip')->first();
            // dd($data);
            return view('frontend.calculators.futurevalueofsip.future_value_of_sip', $data);
        }
    }

    public function futureValueOfSipOutput(Request $request)
    {
        if ($request->isMethod('get')) {
            if (Session::has('future_value_of_sip_form_data')) {
                $input = Session::get('future_value_of_sip_form_data');
                $data['amount'] = $input['amount'];
                $data['period'] = $input['period'];
                $data['interest1'] = $input['interest1'];
                if (isset($input['steuup'])) {
                    $data['stepup'] = $input['steuup'];
                } else {
                    $data['stepup'] = 0;
                }
                ...
                if (isset($input['step_up_amount'])) {
                    $data['step_up_amount'] = $input['step_up_amount'];
                }
                ...
                if (isset($input['step_up_rate']) && $input['step_up_rate'] != '') {
                    $data['step_up_rate'] = $input['step_up_rate'];
                }

                if (isset($input['include_step_up']) && $input['include_step_up'] != '') {
                    $data['include_step_up'] = $input['include_step_up'];
                }
        ...
        if ($request->isMethod('post')) {
            $request->validate([
                ...
                'step_up_rate' => 'sometimes|nullable|numeric|between:1,25',
            ...
            Session::put('future_value_of_sip_form_data', $data);
        }
        ...
        $category_detail = Calculator::where("url", "calculators/sip-calculator/future-value-of-sip")->first();
        ...
        // dd($data);
        session()->forget('pie_chart1');
        session()->forget('pie_chart2');
        $data['edit_id'] = session()->get('future_value_of_sip_new');

        return view('frontend.calculators.futurevalueofsip.future_value_of_sip_output', $data);
    }

    public function futureValueOfSipOutputSave(Request $request)
    {
        ...
        if ($request->isMethod('get')) {
            if (Session::has('future_value_of_sip_form_data')) {
                ...
                $data['pie_chart1'] = Session::get('pie_chart1');
                ...
                $view = (string) View::make('frontend.calculators.futurevalueofsip.future_value_of_sip_output_pdf', $data);
                $edit_id = session()->get('future_value_of_sip_new');
                if ($edit_id) {
    ...
    public function view(Request $request)
    {
        ...
        $data['interest2'] = isset($data['interest2'])?$data['interest2']:null;
        $data['previous_amount_int2'] = isset($data['previous_amount_int2'])?$data['previous_amount_int2']:null;
    
        Session::put('future_value_of_sip_form_data', $data);
        Session::put('pie_chart1', $data['pie_chart1']);
        ...
        $data['details'] = DB::table("calculators")->where('url', 'calculators/sip-calculator/future-value-of-sip')->first();
        return view('frontend.calculators.futurevalueofsip.view', $data);
    }
    ...
    public function merge_download(Request $request)
    {
        ...
        if (!$saveCalculators) {
            return redirect()->route('frontend.one_time_investment_goal_planning_ready_reckoner');
        }
        ...
        if ($data['before_after'] == 'after') {
            $pdf = PDF::loadView('frontend.calculators.futurevalueofsip.future_value_of_sip_output_pdf', $data);
    ...
    public function edit(Request $request)
    {
        ...
        if (isset($saveCalculatorsData['step_up_rate'])) {
            $data['form_data']['step_up_rate'] = $saveCalculatorsData['step_up_rate'];
        }
        if (isset($saveCalculatorsData['step_up_amount'])) {
            $data['form_data']['step_up_amount'] = $saveCalculatorsData['step_up_amount'];
        }
        $data['form_data']['include_step_up'] = $saveCalculatorsData['include_step_up'];
        $data['form_data']['suggest'] = "";
        ...
        Session::put('future_value_of_sip_new', $request->id);
    ...
    public function futureValueOfSipOutputPdfDownload(Request $request)
    {
        if ($request->isMethod('get')) {
            if (Session::has('future_value_of_sip_form_data')) {
                ...
                $data['title'] = "Future Value Report";
