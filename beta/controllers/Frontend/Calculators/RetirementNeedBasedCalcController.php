...
class RetirementNeedBasedCalcController extends Controller
{
    ...
    public function index(Request $request){
        ...
        if($request->action == "back"){
            if (session()->has('retirementPlanning_form_id')){
                session()->forget('retirementPlanning_form_id');
            }
            // dd(Session::get('retirementPlanning_form_data'));
            if (Session::has('retirementPlanning_form_data')) {
                $saveCalculatorsData = Session::get('retirementPlanning_form_data');
                ...
                $data['calculater_heading'] = CalculatorHeading::where('key_name','=','retirement_planning')->first();
                // dd($data);

                $data['details'] = DB::table("calculators")->where("url","calculators/need-based-calculator/retirement-need-based-calculator")->first();
                // dd($data);
                return view('frontend.calculators.retirement_need_based_calc.edit',$data);
                
            }else{
                return redirect()->route('frontend.retirementPlanning');
            }
        }else{
            ...
            if (session()->has('retirementPlanning_form_data')){
                session()->forget('retirementPlanning_form_data');
            }
            if (session()->has('retirementPlanning_form_id')){
                session()->forget('retirementPlanning_form_id');
            }
            ...
            $data['details'] = DB::table("calculators")->where("url","calculators/need-based-calculator/retirement-need-based-calculator")->first();
            // dd($data);
            return view('frontend.calculators.retirement_need_based_calc.index',$data);
        }
    }
    ...
    public function output(Request $request){
        if ($request->isMethod('get')) {
            if (Session::has('retirementPlanning_form_data')) {
                $input = Session::get('retirementPlanning_form_data');
                // dd($input);
                $data = $input;
        ...
        if ($request->isMethod('post')) {
            ...
            $request->validate([
                'distribution_phase_interest_rate_1' => 'required|numeric|between:0.1,18|different:expected_inflation_rate_post_retirement',
                'distribution_phase_interest_rate_2' => 'sometimes|nullable|numeric|between:0.1,18|different:expected_inflation_rate_post_retirement'
            ],[
                    'distribution_phase_interest_rate_1.different' => 'The distribution phase interest rate and expected inflation rate post retirement must be different.',
                    'distribution_phase_interest_rate_2.different' => 'The distribution phase interest rate and expected inflation rate post retirement must be different.'
                ]);
            ...
            Session::put('retirementPlanning_form_data', $data);
        }
        ...
        $category_detail = Calculator::where("url","calculators/need-based-calculator/retirement-need-based-calculator")->first();
        ...
        $edit_id = session()->get('retirementPlanning_form_id');
        if(!empty($edit_id)){
            $calc = SaveCalculators::where('user_id',Auth::user()->id)->where('id',$edit_id)->first();
            $data['cal_name'] = $calc->title;
        }
        
        $calculators = DB::table("calculators")->where("id",28)->first();
        if($calculators){
        ...
        $data['edit_id'] = session()->get('retirementPlanning_form_id');
        // dd(session()->get('calculator_duration'));
        return view('frontend.calculators.retirement_need_based_calc.output',$data);
    }
    ...
    public function save(Request $request){
        ...
        if ($request->isMethod('get')) {
            if (Session::has('retirementPlanning_form_data')) {
                ...
                $view = (string)View::make('frontend.calculators.retirement_need_based_calc.pdf',$data);
    ...
    public function pdf(Request $request){
        if ($request->isMethod('get')) {
            if (Session::has('retirementPlanning_form_data')) {
                ...
                $data['title'] = "One Time Investment Ready Reckoner";

                $data['pie_chart2'] = Session::get('pie_chart2');

                $data['details'] = DB::table("calculators")->where("url","calculators/need-based-calculator/retirement-need-based-calculator")->first();

                if(isset($data['pdf_title_line1']) && $data['pdf_title_line1']){
                    ...
                    $pdf = PDF::loadView('frontend.calculators.retirement_need_based_calc.pdf', $data);
    ...
    public function edit(Request $request){
        ...
        Session::put('retirementPlanning_form_id', $request->id);
    ...
    public function view(Request $request){
        ...
        $data['details'] = DB::table("calculators")->where("url","calculators/need-based-calculator/retirement-need-based-calculator")->first();
        return view('frontend.calculators.retirement_need_based_calc.view',$data);
    }
