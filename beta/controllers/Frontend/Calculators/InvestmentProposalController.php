...
class InvestmentProposalController extends Controller
{
    ...
    public function index(Request $request) {
        ...
        if($request->action == "back"){
            ...
            if (Session::has('investment_proposal')) {
                ...
                $schemecodeData = SchemecodeData::get();

                foreach ($schemecodeData as $key => $value) {
                    $value['data1'] = (array) json_decode($value['data']);
                    array_push($data['scheme_data_list'], $value['data1']);
                }
    ...
    public function output(Request $request){
        ...
        if ($request->isMethod('post')) {
            ...
            $arr_dubli_Schemecode = array();
            $new_schema_data = array();
            $scheme_data_arr = $data['scheme_data_list'];

            foreach ($scheme_data_arr as $key => $value) {
                if (!in_array($value['Schemecode'], $arr_dubli_Schemecode )) {
                    array_push($arr_dubli_Schemecode, $value['Schemecode']);
                    array_push($new_schema_data, $value);
                }
            }
            $data['scheme_data_list'] = $new_schema_data;
