...
class RecoverEmiThroughSIPController extends Controller
{
    ...
    public function index(Request $request)
    {
        if($request->action == "back"){
            if (session()->has('recover_emis_through_sip_form_id')){
                session()->forget('recover_emis_through_sip_form_id');
            }
            // dd(Session::get('recover_emis_through_sip'));
            if (Session::has('recover_emis_through_sip')) {
                $saveCalculatorsData = Session::get('recover_emis_through_sip');
                
                // dd($saveCalculatorsData);
                
                // if (session()->has('recover_emis_through_sip')){
                //     session()->forget('recover_emis_through_sip');
                // }
                ...
                $data['calculater_heading'] = CalculatorHeading::where('key_name','=','recover_emis_through_sip')->first();
                // dd($data);

                $data['details'] = DB::table("calculators")->where("url","premium-calculator/recover_emis_through_sip")->first();
                // dd($data);
                return view('frontend.calculators.recover_emis_through_sip.edit',$data);
                
            }else{
                return redirect()->route('frontend.recover_emis_through_sip');
            }
        }else{
            ...
            $data = [];
            $data['calculater_heading'] = CalculatorHeading::where('key_name','=','recover_emis_through_sip')->first();
            return view('frontend.calculators.recover_emis_through_sip.index',$data); 
    ...
    public function output(Request $request)
    {
        if ($request->isMethod('get')) {
            if (Session::has('recover_emis_through_sip')) {
                $input = Session::get('recover_emis_through_sip');
        ...
        if ($request->isMethod('post')) {
            ...
            $input['expected_interest'] = isset($input['expected_interest'])?$input['expected_interest']:0;

            if($input['enter_loan_details'] == 1){
                $request->validate([
                    'amount' => 'required|numeric|between:100,9999999999',
                    'period' => 'required|numeric|between:1,99',
                    'interest' => 'required|numeric|between:0.1,18'
                ]);
            }else{
                $request->validate([
                    'amount' => 'required|numeric|between:100,9999999999',
            ...
            Session::put('recover_emis_through_sip', $data);
        }
        ...
        $category_detail = Calculator::where("url","premium-calculator/recover_emis_through_sip")->first();
        ...
        $edit_id = session()->get('recover_emis_through_sip_form_id');
        if(!empty($edit_id)){
            $calc = SaveCalculators::where('user_id',Auth::user()->id)->where('id',$edit_id)->first();
            $data['cal_name'] = $calc->title;
        }
        
        $calculators = DB::table("calculators")->where("id",34)->first();
        if($calculators){
        ...
        $data['edit_id'] = session()->get('recover_emis_through_sip_form_id');

        return view('frontend.calculators.recover_emis_through_sip.output',$data);
    }

    public function pdf(Request $request){
        if ($request->isMethod('get')) {
            if (Session::has('recover_emis_through_sip')) {
                $data = Session::get('recover_emis_through_sip');
                ...
                if($data['pdf_title_line1']){
                    ...
                    $pdf = PDF::loadView('frontend.calculators.recover_emis_through_sip.pdf', $data);
    ...
    public function save(Request $request){
        ...
        if ($request->isMethod('get')) {
            if (Session::has('recover_emis_through_sip')) {
                ...
                $view = (string)View::make('frontend.calculators.recover_emis_through_sip.pdf',$data);
                $edit_id = session()->get('recover_emis_through_sip_form_id');
    ...
    public function edit(Request $request){
        ...
        // dd($saveCalculators->title);
        Session::put('calc_title', $saveCalculators->title);
        Session::put('recover_emis_through_sip_form_id', $request->id);
    ...
    public function view(Request $request){
        ...
        $data['details'] = DB::table("calculators")->where('url','premium-calculator/recover_emis_through_sip')->first();
        return view('frontend.calculators.recover_emis_through_sip.view',$data);
    }
