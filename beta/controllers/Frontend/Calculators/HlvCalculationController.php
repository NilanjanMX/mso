...
class HlvCalculationController extends Controller
{
    ...
    public function index(Request $request){
        ...
        if($request->action == "back"){
            if (session()->has('hlv_calculation_form_id')){
                session()->forget('hlv_calculation_form_id');
            }
            // dd(Session::get('hlv_calculation_form_data'));
            if (Session::has('hlv_calculation_form_data')) {
                $saveCalculatorsData = Session::get('hlv_calculation_form_data');
                ...
                $data['calculater_heading'] = CalculatorHeading::where('key_name','=','hlv_calculation')->first();
                // dd($data);

                $data['details'] = DB::table("calculators")->where("url","premium-calculator/hlv_calculation")->first();
                // dd($data);
                return view('frontend.calculators.hlv_calculation.edit',$data);
                
            }else{
                return redirect()->route('frontend.hlv_calculation');
            }
        }else{
            ...
            if (session()->has('hlv_calculation_form_data')){
                session()->forget('hlv_calculation_form_data');
            }
            if (session()->has('hlv_calculation_form_id')){
                session()->forget('hlv_calculation_form_id');
            }
            ...
            $data['details'] = DB::table("calculators")->where("url","premium-calculator/hlv_calculation")->first();
            // dd($data);
            return view('frontend.calculators.hlv_calculation.index',$data);
        }
    }
    ...
    public function output(Request $request){
        if ($request->isMethod('get')) {
            if (Session::has('hlv_calculation_form_data')) {
                $input = Session::get('hlv_calculation_form_data');
                // dd($input);
                $data = $input;
        ...
        if ($request->isMethod('post')) {
            ...
            if($request->formType == 1){
                $data = ['current'=>$request->current,'discount'=>$request->discount,'anual'=>$request->anual,'expected'=>$request->expected,'retire'=>$request->retire,'personal'=>$request->personal,'formType'=>$request->formType,'clientname'=>$request->clientname];
            }
            else
            {
                $count =0;
                $fin = 'financial'.$count;
                $amtReq = 'requiredAmount'.$count;
                $yrslft = 'yearsLeft'.$count;
                $newDat = [];
                $input = $request->all();
                while(!empty($request->$fin) && !empty($request->$amtReq) && !empty($request->$yrslft))
                {
                    $newDat[$count] = [$request->$fin,$request->$amtReq,$request->$yrslft];
                    $count++;
                    $fin = 'financial'.$count;
                    $amtReq = 'requiredAmount'.$count;
                    $yrslft = 'yearsLeft'.$count;
            ...
            Session::put('hlv_calculation_form_data', $data);
        }
        ...
        $category_detail = Calculator::where("url","premium-calculator/hlv_calculation")->first();
        ...
        $data['edit_id'] = session()->get('hlv_calculation_form_id');
        // dd(session()->get('calculator_duration'));
        return view('frontend.calculators.hlv_calculation.output',$data);
    }
    ...
    public function save(Request $request){
        ...
        if ($request->isMethod('get')) {
            if (Session::has('hlv_calculation_form_data')) {
                ...
                $view = (string)View::make('frontend.calculators.hlv_calculation.pdf',$data);
        ...
        if ($request->isMethod('get')) {
            if (Session::has('hlv_calculation_form_data')) {
                ...
                if(isset($data['pdf_title_line1']) && $data['pdf_title_line1']){
                    ...
                    $pdf = PDF::loadView('frontend.calculators.hlv_calculation.pdf', $data);
    ...
    public function edit(Request $request){
        ...
        Session::put('hlv_calculation_form_id', $request->id);
    ...
    public function view(Request $request){
        ...
        $data['details'] = DB::table("calculators")->where("url","premium-calculator/hlv_calculation")->first();
        return view('frontend.calculators.hlv_calculation.view',$data);
    }
